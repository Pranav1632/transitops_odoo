...
export default function DriversPage() {
  ...
  const handleEdit = (driver: Driver) => { setEditingDriver(driver); setIsFormOpen(true); };
  const handleCreate = () => { setEditingDriver(null); setIsFormOpen(true); };
  const handleFormSubmit = async (data: CreateDriverInput | UpdateDriverInput) => {
    if (editingDriver) await actions.editDriver(editingDriver.id, data as UpdateDriverInput);
    else await actions.addDriver(data as CreateDriverInput);
  };
  ...
  return (
    <div className="max-w-screen-2xl mx-auto space-y-5">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-zinc-900 dark:text-zinc-50">Driver Management</h1>
          <p className="text-xs text-zinc-400 mt-0.5">Register and manage active drivers, license logs, safety scores, and expirations.</p>
        </div>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden">
        ...
        {/* Data Table */}
        <DriverTable drivers={drivers} loading={loading} onEdit={handleEdit} onDelete={actions.removeDriver} userRole={profile?.role} />
