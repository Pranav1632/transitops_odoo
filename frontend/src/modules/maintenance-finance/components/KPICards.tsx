  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="h-32 bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6 space-y-3 animate-pulse">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {cardItems.map((item, idx) => {
        ...
        return (
          <div
            key={idx}
            className="rounded-2xl border border-zinc-800 bg-zinc-900/40 backdrop-blur-md p-6 flex flex-col justify-between shadow-sm hover:border-zinc-700 transition-all duration-200 hover:-translate-y-0.5 group"
          >
